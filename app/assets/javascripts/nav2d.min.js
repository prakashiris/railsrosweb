// let alterdatax ,alterdatay;
var checkpoints = {};
var names,goalx,goaly,form,newRadio;
var iddata = [];
var NAV2D = NAV2D || {
    REVISION: "0.3.0"
};
NAV2D.ImageMapClientNav = function(a) {
    ...
    function b(a, we) {
        ...
        var d = new ROS2D.NavigationArrow({
            size: 15,
            strokeSize: 1,
            fillColor: createjs.Graphics.getRGB(25, 64, 128, .66),
            pulse: !0
        });
        d.x =a.position.x ,console.log(a.position.x);  d.y = -a.position.y , console.log(a.position.y);  d.rotation = h.rosQuaternionToGlobalTheta(a.orientation), d.scaleX = 1 / h.scaleX, d.scaleY = 1 / h.scaleY, c.rootObject.addChild(d), b.on("result", function() {
        ...
        d.x =a.position.x ,console.log(a.position.x);  d.y = -a.position.y , console.log(a.position.y);  d.rotation = h.rosQuaternionToGlobalTheta(a.orientation), d.scaleX = 1 / h.scaleX, d.scaleY = 1 / h.scaleY, c.rootObject.addChild(d),
    ...
    var j = new ROS2D.NavigationArrow({
        size: 10,
        strokeSize: 1,
        fillColor: createjs.Graphics.getRGB(13, 255, 207, .66),
        pulse: !0
    });
    ...
    if (l.subscribe(function(a) {
        ...
        document.getElementById('goalx').value = a.stageX;
        document.getElementById('goaly').value = a.stageY;
        console.log(a.stageX);
        console.log(a.stageY);
    }),
    document.getElementById('sendgoalbtn').addEventListener("click", function(a) {
        if(iddata.includes(document.getElementById('goalpass').value)){
            names = document.getElementById('goalpass').value;
                console.log(checkpoints[names].goalx);
                console.log(checkpoints[names].goaly);
                // alterdatax = checkpoints[name].goalx;
                // alterdatay = checkpoints[name].goaly;
                // document.getElementById('twod-map').innerHTML('asa');

        var c = h.globalToRos(checkpoints[names].goalx, checkpoints[names].goaly),
            d = new ROSLIB.Pose({
                position: new ROSLIB.Vector3(c)
            });
        b(d,"g")
